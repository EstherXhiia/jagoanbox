        ...
        <div class="col-12">
          ...
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">News & Article</h3>
            ...
            <div class="card-body">
              <button type="button" class="btn btn-primary mb-3" data-toggle="modal" data-target="#addModal" onclick="cr('new', '0')">
                <i class="fas fa-plus"> Add New News/Article!</i>
              </button>
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td class="text-center align-middle">
                      ...
                      <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#mi-modal" onclick="setConf('<?php echo $row['id'];?>', '<?php echo $row['title'];?>')">
  ...
  <div class="modal-dialog modal-lg">
      <div class="modal-content" style=";color: #000;">
        <div class="modal-header justify-content-between" style="border-bottom: 0px solid !important;">
          <h3 class=""><span id="stat_title"></span> News & Article</h3>
          <input type="text" id="id" hidden="hidden" />
        </div>
        <div class="modal-body">
            <form role="form" enctype="multipart/form-data" method="post" action="#">
              <input type="text" name="stat" id="stat" value="" />
              <input type="text" name="stat" id="edit_id" value="" />
              <div class="row">
                <div class="col-sm-6">
                  <!-- text input -->
...
<script>
  ...
  function pass(id){
    ...
    $.ajax({
				type: "GET",
				dataType: "json",
				url: "mod/GetData.php?mod=news&id="+id, //Relative or absolute path to response.php?mod=news file
				// data: data,
  ...
  function subm(){
    ...
    if(stat == "edit"){
      ...
      $.ajax({
  				type: "POST",
  				dataType: "json",
  				url: "mod/UpdateData.php?mod=news", //Relative or absolute path to response.php?mod=news file
  				data: data,
  				success: function(data){
            // alert(data.status);
            if(data.status == "01"){
      ...
      $.ajax({
  				type: "POST",
  				dataType: "json",
  				url: "mod/InsertData.php?mod=news", //Relative or absolute path to response.php?mod=news file
  				data: data,
  				success: function(data){
            // alert(data.status);
            if(data.status == "01"){
...
modalConfirm(function(confirm){
  if(confirm){
    ...
    $.ajax({
        type: "POST",
        dataType: "json",
        url: "mod/UpdateData.php?mod=news&id="+id, //Relative or absolute path to response.php?mod=news file
        data: data,
        success: function(data){
          // alert(data.status);
          if(data.status == "01"){
